...
CierreCalle.service = {
	...
	selectGridCierreCalle:function(thisGrid, record, index){
		...
		Ext.getCmp('btnEditarCierreCalle').setHref('/mobileApps/cierre-calles/registro?token=6df112b7e941caffe79bd04f788d05e6&id='+record.data.idIncidencia);
	...
	anularIncidencia:function(){
		...
		Ext.Msg.confirm('Confirmar', '¿Seguro de eliminar el registro seleccionado?', function(btn, text){
		    if (btn == 'yes'){
		    	...
		    	Util.runAjax({
							url:'/mobileApps/mapa-incidencia/anular-incidencia',
							params:{
